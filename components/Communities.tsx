...
const neighborhoods = [
  ...
  { id: 6, name: 'Lakewood', listings: '11 Active Properties', price: '$1.3M - $7M', img: 'https://images.unsplash.com/photo-1512917774080-9991f1c4c750?q=80&w=600' }, // ✅ ID fixed to 6
];
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {neighborhoods.map((area, i) => (
          <m.div 
            ...
            // Mobile viewport conditions check instantly to bypass heavy stacking execution delays
            transition={{ duration: 0.4, delay: typeof window !== 'undefined' && window.innerWidth < 768 ? 0 : i * 0.05, ease: "easeOut" }}
            // ✅ Hardware acceleration enabled via tailwind transform mechanics
            className="relative h-[380px] sm:h-[420px] md:h-[440px] rounded-xl overflow-hidden cursor-pointer shadow-2xl border border-white/5 group bg-[#161616] transform-gpu transition-all duration-500 hover:-translate-y-2"
          >
            ...
            <img 
              src={area.img} 
              alt={area.name} 
              // Added hardware level configuration attributes
              decoding="async"
              className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 opacity-55 group-hover:opacity-75 will-change-transform" 
              loading="lazy" 
            />
